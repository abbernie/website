...
CA.prototype.init = function(){
	for(var i = 0; i < 8; i++){
		...
		this.gain[i].connect(this.bus);
		this.osc[i].start(0);

		console.log(this.notes[i]);
	}
	this.bus.connect(this.actx.destination);
}

CA.prototype.env = function(source){
	source.gain.linearRampToValueAtTime((Math.random()*0.5), actx.currentTime);
